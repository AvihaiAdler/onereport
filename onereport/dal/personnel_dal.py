...
def construct_statement(order_by: order_attr.PersonnelOrderBy, order: order_attr.Order, /) -> sqlalchemy.Select[tuple]:
  return sqlalchemy.select(model.Personnel).order_by(sqlalchemy.asc(order_by.value)) if order == order_attr.Order.ASC else sqlalchemy.select(model.Personnel).order_by(sqlalchemy.desc(order_by.value))
