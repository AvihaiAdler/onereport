...
def construct_statement(order_by: order_attr.UserOrderBy, order: order_attr.Order, /) -> sqlalchemy.Select[tuple]:
  return sqlalchemy.select(model.User).order_by(sqlalchemy.asc(order_by.value)) if order == order.Order.ASC else sqlalchemy.select(model.User).order_by(sqlalchemy.desc(order_by.value))
