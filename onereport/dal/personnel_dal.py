...
def find_all_active_personnel(order_by: order_attr.PersonnelOrderBy, order: order_attr.Order, /) -> list[model.Personnel]:
  return model.db.session.scalars(construct_statement(order_by, order).filter(model.Personnel.active)).all()

def find_all_active_personnel_by_company(company: misc.Company, order_by: order_attr.PersonnelOrderBy, order: order_attr.Order, /) -> list[model.Personnel]:
...
def find_all_personnel(order_by: order_attr.PersonnelOrderBy, order: order_attr.Order, /) -> list[model.Personnel]:
